...
namespace DrawClient.Views.UserControls
{
    public partial class Canvas : UserControl
    {
        ...
        // Replay: maps actionId -> TextBlock so ReplayUndo/Redo can show/hide text
        private readonly Dictionary<string, UIElement> _replayChildMap = new Dictionary<string, UIElement>();
        ...
        // Lưu các TRANSFORM_SELECTION nhận từ mạng để Phase 3 của RedrawAllFromActions tái áp dụng.
        // Cần thiết vì remote transforms không nằm trong UndoRedoManager nên bị mất sau undo/redo.
        private class NetworkTransformEntry
        {
            public Matrix Matrix;
            public List<string> StrokeGroupIds = new List<string>();
            public List<string> TextActionIds   = new List<string>();
        }
        private readonly List<NetworkTransformEntry> _activeNetworkTransforms = new List<NetworkTransformEntry>();
        ...
        private void FallbackSyncStroke(Stroke stroke, string groupId)
        {
            ...
            try
            {
                if (pts.Count < 2)
                {
                    // Stroke 1 điểm (click nhanh / nét đầu tiên init stylus) —
                    // vẫn gửi 1 segment ngắn để client khác nhận được.
                    var p = pts.Count == 1
                        ? new Point(pts[0].X, pts[0].Y)
                        : new Point(stroke.GetBounds().X, stroke.GetBounds().Y);
                    _viewModel.SendDrawData(p, new Point(p.X + 1.0, p.Y + 1.0));
                    return;
                }
        ...
        private void Canvas_Unloaded(object sender, RoutedEventArgs e)
        {
            ...
            _activeNetworkTransforms.Clear();
            if (_laserTimer != null && _laserTimer.IsEnabled)
            {
                _laserTimer.Stop();
            }
        ...
        private void SyncSelectionTransform()
        {
                ...
                Math.Abs(newBounds.Height - _oldSelectionBounds.Height) > 0.1)
            {
                ...
                // Thu thập TextBlock bị di chuyển — dùng "T:actionId" thay vì index để ổn định khi replay history
                var allTextBlocks = MyCanvas.Children.OfType<TextBlock>().ToList();
                var selectedChildRefs = new List<string>();
                    ...
                    foreach (UIElement el in selElements)
                    {
                        if (el is TextBlock tb)
                        {
                            if (_childToAction.TryGetValue(tb, out var textAct))
                                selectedChildRefs.Add("T:" + textAct.Id);
                            else
                            {
                                int ci = allTextBlocks.IndexOf(tb);
                                if (ci >= 0) selectedChildRefs.Add(ci.ToString());
                            }
                        }
                    }
                }

                if (selectedIdentifiers.Count > 0 || selectedChildRefs.Count > 0)
                {
                    string childData = selectedChildRefs.Count > 0
                        ? string.Join(",", selectedChildRefs) : "";
        ...
        private void RedrawAllFromActions()
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                // Remote DRAW segments được gom theo StrokeGroupId thành 1 stroke liên tục
                // thay vì nhiều đoạn 2-điểm rời — tránh mất đồng bộ sau undo/redo.
                var drawnNativeGroups = new HashSet<string>();
                // groupId → list of ordered segments (for remote strokes)
                var remoteDrawGroups = new Dictionary<string, List<DrawAction>>();
                ...
                foreach (var action in allActions)
                {
                    ...
                    {
                        if (_groupNativeOriginals.TryGetValue(action.StrokeGroupId, out var originals))
                        {
                            // Local native stroke — dùng bản gốc chưa transform
                        ...
                        {
                            // Remote stroke — thu thập tất cả segments theo group
                            if (!remoteDrawGroups.ContainsKey(action.StrokeGroupId))
                                remoteDrawGroups[action.StrokeGroupId] = new List<DrawAction>();
                            remoteDrawGroups[action.StrokeGroupId].Add(action);
                        }
                    }
                    else if (action.ActionType != "DRAW" || string.IsNullOrEmpty(action.StrokeGroupId))
                    {
                ...
                // Render remote stroke groups thành stroke nhiều điểm (giống DrawNetworkLine)
                foreach (var kvp in remoteDrawGroups)
                {
                    ...
                    // Gom StylusPoints từ tất cả segments
                    var spoints = new StylusPointCollection();
                    spoints.Add(new StylusPoint(firstAct.StartPoint.X, firstAct.StartPoint.Y));
                    foreach (var a in acts)
                        spoints.Add(new StylusPoint(a.EndPoint.X, a.EndPoint.Y));

                    // Xây dựng DrawingAttributes theo penType (giống DrawNetworkLine)
                    string penType = firstAct.penType?.Trim();
                    bool isHl = string.Equals(penType, "highlighter", StringComparison.OrdinalIgnoreCase)
                                || (firstAct.Color?.StartsWith("[HL]") == true);
                    bool isFt = string.Equals(penType, "fountain", StringComparison.OrdinalIgnoreCase);
                    string colorStr = firstAct.Color?.Replace("[HL]", "") ?? "#000000";
                    Color parsedColor;
                    try { parsedColor = (Color)ColorConverter.ConvertFromString(colorStr); }
                    catch { parsedColor = Colors.Black; }

                    var da = new DrawingAttributes { Color = parsedColor, IgnorePressure = true, IsHighlighter = isHl };
                    if (isHl)
                    {
                        da.Width = firstAct.Thickness * 1.5; da.Height = firstAct.Thickness * 1.5;
                        da.StylusTip = StylusTip.Rectangle; da.FitToCurve = false;
                    }
                    else if (isFt)
                    {
                        da.StylusTip = StylusTip.Rectangle;
                        da.Width = firstAct.Thickness * 0.8; da.Height = firstAct.Thickness * 1.8;
                        da.FitToCurve = true; da.IsHighlighter = false;
                    }
                    else
                    {
                        da.Width = firstAct.Thickness; da.Height = firstAct.Thickness;
                        da.StylusTip = StylusTip.Ellipse; da.FitToCurve = true;
                    }

                    var stroke = new Stroke(spoints) { DrawingAttributes = da };
                    MyCanvas.Strokes.Add(stroke);
                    // Đăng ký để các segment mới đến sau vẫn nối vào stroke này
                    _networkGroupStrokes[groupId] = stroke;
                    _currentRedrawGroupStrokes[groupId] = new List<Stroke> { stroke };
                    _strokeToAction[stroke] = firstAct;
                ...
                // ── Phase 4: Apply ERASE + DELETE_TEXT sau tất cả TRANSFORM ─────────────────
                foreach (var action in allActions)
                {
                    if (action.IsUndone) continue;
                    if (action.ActionType != "ERASE" && action.ActionType != "DELETE_TEXT") continue;

                    // ERASE: xóa ink strokes
                    if (action.ActionType == "ERASE")
                        DrawSingleAction(action);

                    // ERASE với AffectedActionIds (sửa text) và DELETE_TEXT: xóa TextBlock theo actionId
                    if (action.AffectedActionIds != null)
                    {
                        foreach (var affId in action.AffectedActionIds)
                        {
                            if (_actionIdToChild.TryGetValue(affId, out var child))
                            {
                                MyCanvas.Children.Remove(child);
                                _actionIdToChild.Remove(affId);
                                _childToAction.Remove(child);
                            }
                        }
                    }
                }
        ...
        private void ApplyNetworkTransformEntry(NetworkTransformEntry nt)
        {
            // Áp dụng transform từ TRANSFORM_SELECTION nhận qua mạng lên stroke/text hiện có trên canvas.
            // Được gọi trong Phase 3 của RedrawAllFromActions để không mất vị trí sau undo/redo.
            foreach (var gid in nt.StrokeGroupIds)
            {
                if (_networkGroupStrokes.TryGetValue(gid, out var ns) && MyCanvas.Strokes.Contains(ns))
                {
                    new StrokeCollection { ns }.Transform(nt.Matrix, false);
                }
                else if (_currentRedrawGroupStrokes.TryGetValue(gid, out var strokes))
                {
            ...
            {
                if (_actionIdToChild.TryGetValue(aid, out var child) && MyCanvas.Children.Contains(child))
                {
                    double l = InkCanvas.GetLeft(child); if (double.IsNaN(l)) l = 0;
                    double t = InkCanvas.GetTop(child);  if (double.IsNaN(t)) t = 0;
                    InkCanvas.SetLeft(child, l * nt.Matrix.M11 + nt.Matrix.OffsetX);
                    InkCanvas.SetTop(child,  t * nt.Matrix.M22 + nt.Matrix.OffsetY);
                }
            }
        }
        ...
        private void HandleTransformUndoRedo(DrawAction transformAction, bool isUndo)
        {
            ...
            // Dùng "T:actionId" thay vì index số nguyên để ổn định khi replay từ history
            // (index số nguyên bị lệch nếu có text nào đó bị xóa trước TRANSFORM_SELECTION này)
            var affectedTextRefs = new List<string>();
            foreach (var affId in affectedIds)
            {
                if (_actionIdToChild.ContainsKey(affId))
                    affectedTextRefs.Add("T:" + affId);
            }

            if (affectedStrokes.Count == 0 && affectedTextRefs.Count == 0) return;
            ...
            if (identifiers.Count == 0 && affectedTextRefs.Count == 0) return;
            ...
            string childData = affectedTextRefs.Count > 0 ? string.Join(",", affectedTextRefs) : "";
        ...
        private void ReplayDraw(DrawMessage msg)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                try
                {
                    string groupId = !string.IsNullOrEmpty(msg.strokeGroupId) ? msg.strokeGroupId : msg.actionId;

                    // Accumulate segments into a single stroke per group (mirrors DrawNetworkLine)
                    // so that TRANSFORM_SELECTION can find and move the stroke via _networkGroupStrokes.
                    if (!string.IsNullOrEmpty(groupId) &&
                        _networkGroupStrokes.TryGetValue(groupId, out var existing) &&
                        MyCanvas.Strokes.Contains(existing))
                    {
                        existing.StylusPoints.Add(new StylusPoint(msg.x2, msg.y2));
                        if (!string.IsNullOrEmpty(msg.actionId))
                        {
                            _replayStrokeMap[msg.actionId] = existing;
                    ...
                    if (string.IsNullOrWhiteSpace(colorToUse)) colorToUse = "#000000";
                    ...
                    var da = new DrawingAttributes { Color = parsedColor, IgnorePressure = true, IsHighlighter = isHighlighter };
                    if (isHighlighter)
                    {
                        da.Width = msg.thickness * 1.5; da.Height = msg.thickness * 1.5;
                        da.StylusTip = StylusTip.Rectangle; da.FitToCurve = false;
                    }
                    else if (isFountain)
                    {
                        da.StylusTip = StylusTip.Rectangle;
                        da.Width = msg.thickness * 0.8; da.Height = msg.thickness * 1.8;
                        da.FitToCurve = true; da.IsHighlighter = false;
                    }
                    else
                    {
                        da.Width = msg.thickness; da.Height = msg.thickness;
                        da.StylusTip = StylusTip.Ellipse; da.FitToCurve = true;
                    }
                    ...
                    var stroke = new Stroke(points) { DrawingAttributes = da };
                    MyCanvas.Strokes.Add(stroke);
        ...
        private void ReplayText(DrawMessage msg)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                try
                {
                    ...
                    // Track so ReplayUndo/Redo can show/hide this TextBlock,
                    // and so TRANSFORM_SELECTION can locate it via _actionIdToChild/_childToAction.
                    // _childToAction must also be set so HandleRemoteSelectionTransform can populate
                    // ntEntry.TextActionIds — without it the entry is never added to _activeNetworkTransforms
                    // and ReplayFinished → RedrawAllFromActions Phase 3 cannot re-apply the transform.
                    if (!string.IsNullOrEmpty(msg.actionId))
                    {
                        _replayChildMap[msg.actionId] = tb;
                        _actionIdToChild[msg.actionId] = tb;
                        if (_viewModel != null)
                        {
        ...
        private void ReplayUndo(string actionId)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                else if (_replayChildMap.TryGetValue(actionId, out var child))
                {
                    // Hide undone TextBlock — remove from canvas but keep in map for redo
                    MyCanvas.Children.Remove(child);
                    _actionIdToChild.Remove(actionId);
                    _replayUndoneActions.Add(actionId);
                }
            });
        }
        ...
        private void ReplayRedo(string actionId)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                if (_replayUndoneActions.Contains(actionId))
                {
                    ...
                    {
                        if (!MyCanvas.Children.Contains(child))
                            MyCanvas.Children.Add(child);
                        _actionIdToChild[actionId] = child;
                    }
        ...
        private void ReplayClear()
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                foreach (var id in _replayChildMap.Keys) _actionIdToChild.Remove(id);
                _replayStrokeMap.Clear();
                _replayActionMap.Clear();
                _replayUndoneActions.Clear();
                _replayChildMap.Clear();
                _networkGroupStrokes.Clear();
            });
        }
        ...
        private void UserControl_PreviewMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            if (e.LeftButton == MouseButtonState.Pressed)
            {
                ...
                {
                    // Ưu tiên: đi lên visual tree để tìm InkCanvas.
                    ...
                    // Fallback: nét đầu tiên WPF stylus-plugin init có thể khiến visual tree chưa
                    // có InkCanvas → dùng kiểm tra toạ độ trong bounds của MyCanvas.
                    if (!clickedOnInkCanvas && MyCanvas != null)
                    {
                        Point pos = e.GetPosition(MyCanvas);
                        clickedOnInkCanvas = pos.X >= 0 && pos.Y >= 0
                                          && pos.X <= MyCanvas.ActualWidth
                                          && pos.Y <= MyCanvas.ActualHeight;
                    }
        ...
        private void ClearLocalCanvas()
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                MyCanvas.Strokes.Clear();
                MyCanvas.Children.Clear();
                // _groupNativeStrokes / _groupNativeOriginals are intentionally NOT cleared:
                // RedrawAllFromActions needs them to restore smooth native pen strokes after
                // replay or undo/redo. Stale entries are safe — they're only accessed when
                // UndoRedoManager still has the matching DrawAction (IsUndone=false).
                _strokeToAction.Clear();
                _childToAction.Clear();
                _actionIdToStroke.Clear();
                _actionIdToChild.Clear();
        ...
        private void CommitTextEdit()
        {
            ...
            // 2. Xóa chữ cũ: ghi vào UndoRedoManager + gửi mạng
            ...
            if (existingRef != null)
            {
                ...
                string existingTextActionId = _childToAction.TryGetValue(existingRef, out var exAct) ? exAct.Id : null;
                ...
                // Shared groupId: ERASE + TEXT2 undo/redo cùng 1 bước
                editGroupId = DrawAction.GenerateId();

                var eraseAction = new DrawAction("ERASE", p1, p2, "#ERASE", eraseThickness,
                    ClientSocket.Instance.CurrentUserId, ClientSocket.Instance.CurrentUsername, _viewModel.RoomId)
                {
                    StrokeGroupId = editGroupId,
                    AffectedActionIds = existingTextActionId != null ? new List<string> { existingTextActionId } : null
                };
                _viewModel.UndoRedoManager.AddAction(eraseAction);

                // Gửi ERASE tới mạng (text field mang existingTextActionId cho Client B rebuild AffectedActionIds)
                ClientSocket.Instance.Send(new DrawMessage
                {
                    type = "ERASE",
                    roomId = _viewModel.RoomId,
                    ...
                    actionId = eraseAction.Id,
                    strokeGroupId = editGroupId,
                    text = existingTextActionId
                });

                // Gửi DELETE_TEXT để xóa TextBlock ngay lập tức trên máy kia
            ...
            if (!string.IsNullOrWhiteSpace(text))
            {
                ...
                var textAction = _viewModel.SendText(text, new Point(left, top), 0, 0, fontSize, fontFam, color, editGroupId);
                if (textAction != null)
                {
        ...
        private void DeleteTextEdit()
        {
            ...
            {
                // Ghi vào UndoRedoManager để undo/redo hoạt động đúng
                string textActionId = _childToAction.TryGetValue(existingRef, out var existingAct) ? existingAct.Id : null;
                if (textActionId != null)
                {
                    var deleteAction = new DrawAction("DELETE_TEXT",
                        new Point(posX, posY), new Point(posX, posY),
                        string.Empty, 0,
                        ClientSocket.Instance.CurrentUserId, ClientSocket.Instance.CurrentUsername, _viewModel.RoomId)
                    {
                        AffectedActionIds = new List<string> { textActionId }
                    };
                    _viewModel.UndoRedoManager.AddAction(deleteAction);
                }
        ...
        private StylusPointCollection CreateRectanglePoints(Point start, Point end)
        {
            ...
            for (int i = 0; i <= steps; i++) points.Add(new StylusPoint(x0 + (x1 - x0) * i / steps, y0));
            for (int i = 1; i <= steps; i++) points.Add(new StylusPoint(x1, y0 + (y1 - y0) * i / steps));
            for (int i = 1; i <= steps; i++) points.Add(new StylusPoint(x1 - (x1 - x0) * i / steps, y1));
            for (int i = 1; i <= steps; i++) points.Add(new StylusPoint(x0, y1 - (y1 - y0) * i / steps));
            return points;
        }
        ...
        private StylusPointCollection CreateLinePoints(Point start, Point end)
        {
            ...
            for (int i = 0; i <= steps; i++)
                points.Add(new StylusPoint(start.X + (end.X - start.X) * i / steps, start.Y + (end.Y - start.Y) * i / steps));
            return points;
        }
        ...
        private StylusPointCollection CreateTrianglePoints(Point start, Point end)
        {
            var points = new StylusPointCollection();
            const int steps = 20;
            double topX = start.X + (end.X - start.X) / 2, topY = start.Y;
            double brX = end.X, brY = end.Y;
            double blX = start.X, blY = end.Y;
            for (int i = 0; i <= steps; i++) points.Add(new StylusPoint(topX + (brX - topX) * i / steps, topY + (brY - topY) * i / steps));
            for (int i = 1; i <= steps; i++) points.Add(new StylusPoint(brX + (blX - brX) * i / steps, brY));
            for (int i = 1; i <= steps; i++) points.Add(new StylusPoint(blX + (topX - blX) * i / steps, blY + (topY - blY) * i / steps));
            return points;
        }
        ...
        private void HandleRemoteSelectionTransform(string json)
        {
            ...
            try
            {
                using (var doc = System.Text.Json.JsonDocument.Parse(json))
                {
                    ...
                    // Ghi nhận để Phase 3 của RedrawAllFromActions tái áp dụng sau undo/redo
                    var ntEntry = new NetworkTransformEntry { Matrix = matrix };
                    ...
                    if (!string.IsNullOrWhiteSpace(parts[0]))
                    {
                        foreach (string id in parts[0].Split(','))
                        {
                            ...
                            if (trimmedId.StartsWith("G:"))
                            {
                                ...
                                // Lưu vào entry để Phase 3 biết nhóm nào cần transform
                                if (!ntEntry.StrokeGroupIds.Contains(gid))
                                    ntEntry.StrokeGroupIds.Add(gid);
                                ...
                                if (!addedNet && _currentRedrawGroupStrokes.TryGetValue(gid, out var redrawList))
                                {
                                    foreach (var rs in redrawList)
                                    {
                                        if (MyCanvas.Strokes.Contains(rs))
                                        {
                                            strokesToTransform.Add(rs);
                                            addedNet = true;
                                        }
                                    }
                                }
                    ...
                    // Apply transform cho TextBlock children + lưu actionId để Phase 3 tái áp dụng
                    if (parts.Length >= 4 && !string.IsNullOrWhiteSpace(parts[3]))
                    {
                        var allTbs = MyCanvas.Children.OfType<TextBlock>().ToList();
                        foreach (var idxStr in parts[3].Split(','))
                        {
                            string trimmedRef = idxStr.Trim();
                            TextBlock tb = null;

                            if (trimmedRef.StartsWith("T:"))
                            {
                                // Định dạng mới: "T:actionId" — tra cứu ổn định qua actionId
                                string aid = trimmedRef.Substring(2);
                                if (_actionIdToChild.TryGetValue(aid, out var child) && child is TextBlock textBlock)
                                    tb = textBlock;
                            }
                            else if (int.TryParse(trimmedRef, out int ci) && ci >= 0 && ci < allTbs.Count)
                            {
                                // Định dạng cũ: integer index (backward-compat với data cũ trong DB)
                                tb = allTbs[ci];
                            }
                            ...
                            if (tb != null)
                            {
                                ...
                                if (_childToAction.TryGetValue(tb, out var textAct) &&
                                    ...
                                    ntEntry.TextActionIds.Add(textAct.Id);
                            }
                        }
                    }

                    // Đẩy vào danh sách transforms để Phase 3 của RedrawAllFromActions tái áp dụng.
                    // Cho phép tích luỹ (forward + reverse = identity) nên list không cần xoá từng entry.
                    if (ntEntry.StrokeGroupIds.Count > 0 || ntEntry.TextActionIds.Count > 0)
                        _activeNetworkTransforms.Add(ntEntry);
                }
            }
            catch (Exception ex)
            {
